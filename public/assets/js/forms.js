...
  		}
	...
	var c = setInterval(function(){
		var expire3 = document.getElementsById('receivingact');
		for(var i = 0; i < expire2.length; i++){
			var countDown= new Date(expire2.innerHTML).getTime(  );
			console.log(countDown)
			var now = new Date().getTime(  );
	...
	  		var seconds = Math.floor((distance % (1000 * 60)) / 1000);
		//	var status = 'ACTIVE'
			...
			//show them in the demo
							document.getElementById("receivingactleft").innerHTML = " " + days + "d " + hours + "h "
	  + minutes + "m " + seconds + "s ";
			//document.getElementById("registerbutton").style.display= "none";
			
			if (distance  < 0) {
	    		clearInterval(x);
				document.getElementById('receivingactleft').innerHTML = "EXPIRED";
				//document.getElementById("registerbutton").style.display= "block";
	    		
	  		}
		}
